...
final class TexturePainter extends ViewPainter {
    ...
    private UploadRTTexture   resolveRTT = null;

    private float penScaleX, penScaleY;

    TexturePainter(GlassScene view) {
        super(view);
    }
    ...
    @Override public void run() {
        ...
        try {
            ...
            if (needsReset) {
                disposeRTTexture();
                if (sceneState.isMSAA()) {
                    rttexture = factory.createRTTexture(bufWidth, bufHeight, WrapMode.CLAMP_NOT_NEEDED,
                            true);
                } else {
                    rttexture = factory.createUploadRTTexture(bufWidth, bufHeight);
                }
                if (rttexture == null) {
                    return;
                }
    ...
    private UploadRTTexture resolveRenderTarget(Graphics g, int width, int height) {
        if (resolveRTT != null) {
            resolveRTT.lock();
        ...
        if (resolveRTT == null) {
            resolveRTT = g.getResourceFactory().createUploadRTTexture(width, height);
        }
